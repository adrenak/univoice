...
namespace Adrenak.UniVoice {
    public class DefaultAudioOutputFactory : IAudioOutputFactory {
        public IAudioOutput Create
        (int samplingRate, int channelCount, int segmentLength) 
        {
            return DefaultAudioOutput.New(
                new AudioBuffer(samplingRate, channelCount, segmentLength, 5),
                new GameObject($"UniVoice Peer").AddComponent<AudioSource>(),
                3
            );
        }

        public void Destroy(IAudioOutput audioOutput) =>
            Object.Destroy((audioOutput as DefaultAudioOutput).gameObject);
    }
    ...
    public class DefaultAudioOutput : MonoBehaviour, IAudioOutput {
        ...
        public string ID {
            ...
            set {
                ...
                AudioBuffer.AudioClip.name = value;
            }
        }
        ...
        /// between 1 and <see cref="AudioBuffer.SegCount"/> of the 
        /// <see cref="AudioBuffer"/> passed.
        /// Default: 0. Results in the value being set to the max possible.
        /// </param>
        public static DefaultAudioOutput New
        (AudioBuffer buffer, AudioSource source, int minSegCount = 0) 
        {
            var ctd = source.gameObject.AddComponent<DefaultAudioOutput>();
